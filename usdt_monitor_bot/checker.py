...
class TransactionChecker:
    ...
    async def _enrich_transaction_metadata(
        ...
        try:
            ...
            with open(log_path, "a") as f:
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"G","location":"checker.py:430","message":"Before risk analysis","data":{"tx_hash":tx_metadata.tx_hash,"from_address":tx_metadata.from_address,"to_address":tx_metadata.to_address,"value":str(tx_metadata.value),"value_decimal":float(tx_metadata.value),"is_zero":tx_metadata.value == 0,"whitelist_size":len(whitelisted_addresses),"from_in_whitelist":tx_metadata.from_address.lower() in {a.lower() for a in whitelisted_addresses},"to_in_whitelist":tx_metadata.to_address.lower() in {a.lower() for a in whitelisted_addresses}},"timestamp":int(time.time()*1000)}) + "\n")
        except Exception:
            pass
        # #endregion
        ...
        try:
            ...
            if str(current) == "/app" or (len(current.parts) > 0 and current.parts[-1] == "app"):
                ...
                log_path = str(Path.cwd() / ".cursor" / "debug.log")
            with open(log_path, "a") as f:
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"G","location":"checker.py:435","message":"After risk analysis","data":{"tx_hash":tx_metadata.tx_hash,"risk_score":risk_analysis.score,"is_suspicious":risk_analysis.is_suspicious,"flags":[f.value for f in risk_analysis.flags],"details":risk_analysis.details},"timestamp":int(time.time()*1000)}) + "\n")
        except Exception:
            pass
        # #endregion
    ...
    async def _process_single_transaction(
        ...
        try:
            ...
            with open(log_path, "a") as f:
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"F","location":"checker.py:504","message":"Token config lookup","data":{"tx_hash":tx_hash,"token_symbol":tx_token_symbol,"has_token_config":token_config is not None,"spam_detection_enabled":self._spam_detection_enabled},"timestamp":int(time.time()*1000)}) + "\n")
        except Exception:
            pass
        # #endregion
        if not token_config:
        ...
        if token_config and self._spam_detection_enabled:
            ...
            try:
                ...
                if str(current) == "/app" or (len(current.parts) > 0 and current.parts[-1] == "app"):
                    ...
                    log_path = str(Path.cwd() / ".cursor" / "debug.log")
                with open(log_path, "a") as f:
                    f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"F","location":"checker.py:520","message":"Transaction metadata created","data":{"tx_hash":tx_hash,"has_metadata":tx_metadata is not None,"value":str(tx_metadata.value) if tx_metadata else None,"value_type":type(tx_metadata.value).__name__ if tx_metadata else None},"timestamp":int(time.time()*1000)}) + "\n")
            except Exception:
                pass
            # #endregion
            if tx_metadata:
                ...
                try:
                    ...
                    if str(current) == "/app" or (len(current.parts) > 0 and current.parts[-1] == "app"):
                        ...
                        log_path = str(Path.cwd() / ".cursor" / "debug.log")
                    with open(log_path, "a") as f:
                        f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"F","location":"checker.py:524","message":"Risk analysis result","data":{"tx_hash":tx_hash,"has_risk_analysis":risk_analysis is not None,"risk_score":risk_analysis.score if risk_analysis else None,"is_suspicious":risk_analysis.is_suspicious if risk_analysis else None,"flags":[f.value for f in risk_analysis.flags] if risk_analysis and risk_analysis.flags else []},"timestamp":int(time.time()*1000)}) + "\n")
                except Exception:
                    pass
                # #endregion
